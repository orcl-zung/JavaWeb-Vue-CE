...
<script>
import {unpassSort} from '../../plugins/api' //审核不通过
import {passSort} from '../../plugins/api' //审核通过
import {uncheckSort} from '../../plugins/api' //管理员未审核订单
import {checkPasSort} from '../../plugins/api' //管理员审核通过订单
import {checkunPasSort} from '../../plugins/api' //管理员审核未通过订单
export default {
    name:'sort',
    data(){
      return{
    ...
    methods: {
      ...
      handleClick(tab, event) {
        ...
        if(name == 'first'){
          ...
          //审核通过
          checkPasSort().then(function(res){
            if(res.data.code == 'ok'){
              var data = res.data.data
          ...
          //审核未通过
          checkunPasSort().then(function(res){
            if(res.data.code == 'ok'){
              var data = res.data.data
